...
function App() {
  ...
  const onClickGenerate = async () => {
    if (startDate > endDate || commits < 0 || frequency < 0 || endDate > new Date() || commits > MAX_COMMIT || frequency > MAX_FREQUENCY) {
      alert("Invalid data");
    }
      ...
      <div className="w-1/2 bg-white p-10 rounded-lg shadow-lg">
        ...
        <div className="grid grid-cols-2 gap-5">
          <div>
            ...
            <input type="date" value={startDate.toISOString().slice(0, 10)} onChange={onChangeStartDate} className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required />
          </div>
          <div>
            <label className="block text-gray-700 text-sm font-bold mb-2">End Date:</label>
            <input type="date" value={endDate.toISOString().slice(0, 10)} onChange={onChangeEndDate} className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required />
          </div>
        </div>
        <div className="grid grid-cols-2 gap-5 mt-5">
          <div>
            <label className="block text-gray-700 text-sm font-bold mb-2">Commits:</label>
            <input type="number" value={commits} onChange={onChangeCommits} className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required />
          </div>
          <div>
            <label className="block text-gray-700 text-sm font-bold mb-2">Frequency:</label>
            <input type="number" value={frequency} onChange={onChangeFrequency} className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required />
          </div>
        </div>
